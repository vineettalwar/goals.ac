...
export async function syncGscSearchAnalytics(
  ...
  const { maybeRefreshBrandAfterGscSync } = await import("./support/brand-scrape-orchestrator");
...
export async function sweepGscSyncProjects(): Promise<void> {
  ...
  const { discoverOpportunities } = await import("./keyword-opportunity-service");
