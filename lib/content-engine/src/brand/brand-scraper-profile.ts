...
export type ScrapeBrandProfileOptions = {
  ...
  refresh?: boolean;
  /** Project owner — scrape uses their org BYOK provider (Ollama, OpenAI, Anthropic, Bedrock, Gemini). */
  userId?: number;
};
...
async function brandAiClient(userId?: number): Promise<AiProviderClient> {
  if (userId != null) {
...
export async function scrapeBrandProfile(
  ...
  if (!homepageHtml && crawledPages.length === 0) {
  ...
  const ai = await brandAiClient(options?.userId);
...
Banned in all string fields: placeholder URLs, example.com, "competitor1", buzzwords (synergy, leverage, cutting-edge, world-class, outcome-focused, results-oriented, seamless, robust, holistic, transformative, game-changer)`;
  ...
  for (let attempt = 1; attempt <= 2; attempt++) {
    try {
      ...
      const extract = sanitizeBrandExtract({
      ...
      try {
        extract.deep = await analyzeBrandVoiceDeep(
          websiteUrl,
          allText,
          writingSamples,
          scannedPages,
        ...
        logger.warn({ err }, "Brand deep extract failed; saving core profile");
      }
