...
export async function processBrandVoiceIndex(payload: BrandVoiceIndexJobData): Promise<void> {
  const { projectId, sourceIds, regenerateSkill } = payload;
...
export async function registerBrandVoiceIndexHandler(boss: PgBoss): Promise<void> {
  await boss.work<BrandVoiceIndexJobData>(QUEUES.brandVoiceIndex, async ([job]) => {
    await processBrandVoiceIndex(job.data);
  });
}
