...
/**
 * Stores and injects schema.org JSON-LD and llms.txt content.
 */
class Schema_Inject {
	...
	/**
	 * Register frontend hooks for schema injection.
	 */
			...
			if ( \is_string( $schema ) ) {
				...
				if ( JSON_ERROR_NONE !== \json_last_error() ) {
					return new \WP_Error(
						...
						/* translators: %d: JSON-LD schema index in the request payload. */
	...
	public function inject_json_ld(): void {
		...
		$schemas = \get_option( self::SCHEMA_OPTION, array() );
		if ( empty( $schemas ) ) {
			return;
		}
	...
	/**
	 * Reserved hook for async-loaded schema output.
	 */
	...
	/**
	 * Register the /llms.txt rewrite endpoint.
	 */
