...
export async function handleAuthOpenaiWrite(
  request: Request,
  path: string,
  userId: number,
): Promise<Response | null> {
  if (path === "/api/auth/openai-credentials/test" && request.method === "POST") {
    ...
    try {
      ...
      const client = OpenAIClient.create({ apiKey: parsed.data.key });
  ...
  if (path === "/api/auth/openai-credentials" && request.method === "PATCH") {
    ...
    await db
      ...
      .set({ encryptedOpenaiApiKey: encrypted })
  ...
  if (path === "/api/auth/openai-credentials" && request.method === "DELETE") {
    const forbidden = await requireSiteAdmin(request, userId);
    if (forbidden) return forbidden;
    ...
    await db
      .update(organizationsTable)
      .set({ encryptedOpenaiApiKey: null })
      .where(eq(organizationsTable.id, orgSettings.organizationId));
