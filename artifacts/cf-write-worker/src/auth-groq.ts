...
export async function handleAuthGroqWrite(
  request: Request,
  path: string,
  userId: number,
): Promise<Response | null> {
  if (path === "/api/auth/groq-credentials/test" && request.method === "POST") {
    ...
    try {
      ...
      const client = GroqClient.create({ apiKey: parsed.data.key });
  ...
  if (path === "/api/auth/groq-credentials" && request.method === "PATCH") {
    ...
    await db
      ...
      .set({ encryptedGroqApiKey: encrypted })
  ...
  if (path === "/api/auth/groq-credentials" && request.method === "DELETE") {
    const forbidden = await requireSiteAdmin(request, userId);
    if (forbidden) return forbidden;
