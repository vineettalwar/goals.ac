...
export async function handleAuthAnthropicWrite(
  request: Request,
  path: string,
  userId: number,
): Promise<Response | null> {
  if (path === "/api/auth/anthropic-credentials/test" && request.method === "POST") {
  ...
  if (path === "/api/auth/anthropic-credentials" && request.method === "PATCH") {
  ...
  if (path === "/api/auth/anthropic-credentials" && request.method === "DELETE") {
    const forbidden = await requireSiteAdmin(request, userId);
    if (forbidden) return forbidden;
