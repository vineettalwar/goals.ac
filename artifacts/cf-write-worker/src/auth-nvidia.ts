...
export async function handleAuthNvidiaWrite(
  request: Request,
  path: string,
  userId: number,
): Promise<Response | null> {
  if (path === "/api/auth/nvidia-credentials/test" && request.method === "POST") {
    ...
    try {
      ...
      const client = NvidiaClient.create({ apiKey: parsed.data.key });
  ...
  if (path === "/api/auth/nvidia-credentials" && request.method === "PATCH") {
    ...
    await db
      ...
      .set({ encryptedNvidiaApiKey: encrypted })
  ...
  if (path === "/api/auth/nvidia-credentials" && request.method === "DELETE") {
    const forbidden = await requireSiteAdmin(request, userId);
    if (forbidden) return forbidden;
    ...
    await db
      .update(organizationsTable)
      .set({ encryptedNvidiaApiKey: null })
      .where(eq(organizationsTable.id, orgSettings.organizationId));
