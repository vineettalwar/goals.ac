...
export async function handleAuthOpenrouterWrite(
  request: Request,
  path: string,
  userId: number,
): Promise<Response | null> {
  if (path === "/api/auth/openrouter-credentials/test" && request.method === "POST") {
    ...
    try {
      ...
      const client = OpenRouterClient.create({ apiKey: parsed.data.key });
  ...
  if (path === "/api/auth/openrouter-credentials" && request.method === "PATCH") {
    ...
    await db
      ...
      .set({ encryptedOpenrouterApiKey: encrypted })
  ...
  if (path === "/api/auth/openrouter-credentials" && request.method === "DELETE") {
    const forbidden = await requireSiteAdmin(request, userId);
    if (forbidden) return forbidden;
    ...
    await db
      .update(organizationsTable)
      .set({ encryptedOpenrouterApiKey: null })
      .where(eq(organizationsTable.id, orgSettings.organizationId));
