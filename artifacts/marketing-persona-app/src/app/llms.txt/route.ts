...
export function GET() {
  ...
  const lines = [
    "# goals.ac",
    "",
    "> Cross-platform content studio — publish to WordPress, Ghost, Shopify, and other CMS/social destinations. Deep plugin and Admin API paths for primary stacks; Basic publish for headless and site builders.",
    "",
    "## Product",
    `- [Home](${base}/): Overview of goals.ac`,
    `- [Content Studio](${base}/content-engine): Research, draft, and publish workflow`,
    `- [Plans & pricing](${base}/pricing): Hands-on GEO/AEO programs on /pricing. Self-serve Starter (free) / Growth are available in-app after signup (Settings → Billing) — Growth is $49/mo when Stripe is configured.`,
