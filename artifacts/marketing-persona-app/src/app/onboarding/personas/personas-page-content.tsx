...
import { queryKeys, useOnboardingPersonas } from "@/lib/queries";
...
export function PersonasPageContent({ companyId }: { companyId: string }) {
  ...
  const {
    ...
    isLoading: loading,
    isError,
  } = useOnboardingPersonas(companyId);
  ...
  async function generatePersonas() {
    await queryClient.invalidateQueries({ queryKey: queryKeys.onboardingPersonas(companyId) });
  }
  ...
  async function handleUpdate(id: number, field: string, value: string | string[]) {
    ...
    queryClient.setQueryData<Persona[]>(queryKeys.onboardingPersonas(companyId), (prev = []) =>
      prev.map((p) => (p.id === id ? { ...p, ...persona } : p)),
    );
  }
...
export function PersonasPageClient({ companyId }: { companyId: string }) {
  return (
    <Suspense fallback={<PersonasPageFallback />}>
      <PersonasPageContent companyId={companyId} />
    </Suspense>
  );
}
