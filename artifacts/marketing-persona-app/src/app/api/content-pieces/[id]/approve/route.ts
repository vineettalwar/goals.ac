...
import { approvePiece } from "@workspace/content-engine/support/social/social-queue-service";
...
export async function POST(
  ...
  if (!siteAdmin) {
    return NextResponse.json({ error: "Site admin access required to approve" }, { status: 403 });
  }
  ...
  const updated = await approvePiece(id, userId!);
