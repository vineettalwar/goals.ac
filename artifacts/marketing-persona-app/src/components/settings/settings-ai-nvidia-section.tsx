...
export function SettingsAiNvidiaSection({
  canManage,
  initialHasKey = false,
  ...
  return (
    <>
      <div className="paper-card p-6 space-y-4">
        <h2 className="font-semibold">NVIDIA NIM BYOK</h2>
        <p className="text-sm text-muted-foreground">
          Bring your own NVIDIA API key from build.nvidia.com. Trial keys are rate-limited; not every catalog entry is a chat model.
        ...
        {hasKey ? (
          <div className="space-y-3">
            <div className="flex items-center gap-3 p-3 rounded-lg bg-emerald-500/10 border border-emerald-500/20">
              ...
              <div>
                <p className="text-sm font-medium">Organization NVIDIA API key connected</p>
          ...
          <Button variant="outline" size="sm" onClick={openDialog}>
            <KeyRound className="mr-2 h-3.5 w-3.5" />Add NVIDIA API key
          </Button>
        ) : (
          <p className="text-sm text-muted-foreground">No organization NVIDIA API key configured.</p>
        )}
      </div>
      ...
      <Dialog open={dialogOpen} onOpenChange={setDialogOpen}>
        <DialogContent>
          <DialogHeader>
            <DialogTitle>NVIDIA API key</DialogTitle>
            <DialogDescription>
              Create a key at build.nvidia.com/settings. Your key is encrypted and stored securely.
            </DialogDescription>
          </DialogHeader>
          <div className="space-y-3">
            <Input
