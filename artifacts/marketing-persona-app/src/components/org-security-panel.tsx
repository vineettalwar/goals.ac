"use client";

import { useEffect, useState } from "react";
import { toast } from "sonner";
import { Button } from "@/components/ui/button";
import { Input } from "@/components/ui/input";
import { Label } from "@/components/ui/label";
import { Switch } from "@/components/ui/switch";

export function OrgSecurityPanel({ canManage }: { canManage: boolean }) {
  const [requireMfa, setRequireMfa] = useState(false);
  const [allowedIps, setAllowedIps] = useState("");
  const [maxSessionAgeHours, setMaxSessionAgeHours] = useState("");
  const [allowCrossProjectEditors, setAllowCrossProjectEditors] = useState(false);
  const [saving, setSaving] = useState(false);

  useEffect(() => {
    if (!canManage) return;
    void fetch("/api/organizations/security")
      .then((r) => (r.ok ? r.json() : null))
      .then((data: { securitySettings?: Record<string, unknown> } | null) => {
        if (!data?.securitySettings) return;
        setRequireMfa(Boolean(data.securitySettings.requireMfa));
        setAllowCrossProjectEditors(Boolean(data.securitySettings.allowCrossProjectEditors));
        const ips = data.securitySettings.allowedIps;
        if (Array.isArray(ips)) setAllowedIps(ips.join("\n"));
        const maxAge = data.securitySettings.maxSessionAgeHours;
        if (typeof maxAge === "number") setMaxSessionAgeHours(String(maxAge));
      })
      .catch(() => undefined);
  }, [canManage]);

  async function save() {
    setSaving(true);
    try {
      const res = await fetch("/api/organizations/security", {
        method: "PATCH",
        headers: { "Content-Type": "application/json" },
        body: JSON.stringify({
          requireMfa,
          allowCrossProjectEditors,
          allowedIps: allowedIps
            .split("\n")
            .map((line) => line.trim())
            .filter(Boolean),
          maxSessionAgeHours: maxSessionAgeHours ? Number(maxSessionAgeHours) : undefined,
        }),
      });
      if (!res.ok) throw new Error("Save failed");
      toast.success("Security settings updated");
    } catch {
      toast.error("Failed to save security settings");
    } finally {
      setSaving(false);
    }
  }

  if (!canManage) return null;

  return (
    <div className="paper-card p-5 space-y-4">
      <div>
        <h2 className="text-sm font-semibold">Organization security</h2>
        <p className="text-xs text-muted-foreground mt-1">
          IP allowlist, MFA requirement, and session policies for your org.
        </p>
      </div>
      <div className="flex items-center justify-between gap-4">
        <Label htmlFor="require-mfa">Require MFA (when enabled per user)</Label>
        <Switch id="require-mfa" checked={requireMfa} onCheckedChange={setRequireMfa} />
      </div>
      <div className="flex items-center justify-between gap-4">
        <Label htmlFor="cross-project">Editors can access all projects</Label>
        <Switch
          id="cross-project"
          checked={allowCrossProjectEditors}
          onCheckedChange={setAllowCrossProjectEditors}
        />
      </div>
      <div className="space-y-1.5">
        <Label htmlFor="allowed-ips">IP allowlist (one per line)</Label>
        <Input
          id="allowed-ips"
          value={allowedIps}
          onChange={(e) => setAllowedIps(e.target.value)}
          placeholder="203.0.113.10"
        />
      </div>
      <div className="space-y-1.5">
        <Label htmlFor="session-age">Max session age (hours)</Label>
        <Input
          id="session-age"
          type="number"
          min={1}
          max={720}
          value={maxSessionAgeHours}
          onChange={(e) => setMaxSessionAgeHours(e.target.value)}
          placeholder="168"
        />
      </div>
      <Button size="sm" onClick={() => void save()} disabled={saving}>
        Save security settings
      </Button>
    </div>
  );
}
