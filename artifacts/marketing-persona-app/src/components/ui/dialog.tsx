...
function DialogContent({
  ...
  return (
    <DialogPortal>
      <DialogOverlay className={fullscreen ? "bg-background" : undefined} />
      <DialogPrimitive.Content
        className={cn(
          fullscreen
            ? [
                "fixed inset-0 z-50 flex flex-col w-full max-w-none h-dvh translate-x-0 translate-y-0",
                "rounded-none border-0 p-0 shadow-none",
                "data-[state=open]:animate-in data-[state=closed]:animate-out",
                "data-[state=closed]:fade-out-0 data-[state=open]:fade-in-0",
              ]
            : [
                "fixed left-1/2 top-1/2 z-50 grid w-full max-w-lg -translate-x-1/2 -translate-y-1/2 gap-4 bg-background p-6 duration-200",
