...
function UrlTool({ api, title }: { api: string; title: string }) {
  ...
  return (
    <div className="paper-card p-6 space-y-4">
      ...
      {result && <pre className="text-xs bg-secondary/50 rounded-lg p-4 overflow-x-auto max-h-64">{result}</pre>}
    </div>
  );
}
        ...
        <div className="grid md:grid-cols-2 gap-6">
          {TOOLS.map((tool) => (
            <div key={tool.id} id={tool.id} className="scroll-mt-24">
              {tool.href ? (
                <Link href={tool.href} className="paper-card paper-card-hover p-6 block group">
