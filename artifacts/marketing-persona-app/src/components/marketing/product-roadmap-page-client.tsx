        ...
        <ul className="space-y-2 max-w-2xl mx-auto mb-12">
          {SHIPPED.map((item) => (
            <li key={item.name} className="text-sm text-center">
              <Link href={item.href} className="text-primary hover:underline">{item.name}</Link>
            </li>
          ))}
        </ul>
            ...
            <li key={item.name} className={`${glassCard} p-4 flex items-center justify-between gap-4`}>
              <div className="flex items-center gap-3">
                ...
                {"href" in item && item.href ? (
                  <Link href={item.href} className="font-medium text-white hover:text-primary">{item.name}</Link>
                ) : (
