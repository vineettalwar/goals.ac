...
import { AlertCircle, CheckCircle2, Circle, Loader2, Shuffle } from "lucide-react";
import {
  Dialog,
  DialogContent,
  DialogDescription,
...
export function ContentPieceRepurposeDialog({ open, onClose, pieceId, projectId, currentFormat }: Props) {
  ...
  const handleRepurpose = async () => {
    ...
    try {
      const res = await fetch(`/api/content-pieces/${pieceId}/repurpose/stream`, {
        method: "POST",
    ...
    <Dialog open={open} onOpenChange={(v) => !v && handleClose()}>
      <DialogContent className="max-w-lg">
        <DialogHeader>
          ...
          <DialogDescription>
            Convert this {currentLabel} into a different format. A new content piece will be created.
          </DialogDescription>
        </DialogHeader>
        ...
        <div className="space-y-4 mt-2">
          <div className="space-y-1.5">
            ...
            <Select
              value={targetFormat}
              onValueChange={(v) => setTargetFormat(v as ContentFormatType)}
