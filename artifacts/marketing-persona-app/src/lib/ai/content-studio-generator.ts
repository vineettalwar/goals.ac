...
/**
 * Gemini occasionally emits raw C0 control characters inside JSON string values.
 * This sanitizes them so JSON.parse succeeds.
 */
...
export async function generateContentPiece(
  ...
  return generateWithClient(ai, format, brand, keyword, angleHint);
}
...
export async function generateContentPieceStream(
  ...
  const ai = getAiClient();
  return generateWithClientStream(ai, format, brand, keyword, onChunk, angleHint);
}
