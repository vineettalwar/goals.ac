...
**Internal linking** is the white-hat alternative: connect pillar pages to supporting articles with contextual anchors, fix orphan pages, and complete topical clusters. goals.ac Internal Link Hub (Beta) maps your site graph and suggests links per draft, building authority you own.`,
  },
  {
    ...
    category: "Content strategy",
    cta: { label: "See the content studio", href: "/content-engine" },
    body: `Autopilot SEO tools optimize for article volume: 30 posts per month, auto-published, minimal review. That works for some niches, but B2B teams often need:

- A **coherent narrative** tied to product launches and market moves
- **Editorial control** before brand content goes live
- **Inspectable outputs:** quality scores, citations, schema

goals.ac starts with keyword and competitor research, turns it into a 30-day calendar, then generates brief-driven drafts you approve and publish cross-platform. Autopilot is optional, not mandatory.`,
  },
  {
...
goals.ac Topical Map shows cluster coverage, quick-win keywords, and recommended next articles, tied to your content calendar and Autopilot queue.`,
  },
  {
...
AI systems and Google use structured data to understand entities, answers, and citations. Run the free GEO audit to find missing schema on existing pages.`,
  },
  {
    ...
    body: `Organization admins can add a Semrush API key in **Integrations → AI**. Gap discovery caches results 24h to conserve API units.
...
Endpoints live at \`/api/v1/\` for connections, content render, and content-piece publish. Ideal for custom CMS pipelines during engagements.`,
  },
  {
...
Agency white-label and reseller billing are on the roadmap. Sign up to manage multiple client projects today.`,
  },
  {
...
export function getLearnPostsByCategory(category: LearnCategory): LearnPost[] {
  return LEARN_POSTS.filter((p) => p.category === category);
}
