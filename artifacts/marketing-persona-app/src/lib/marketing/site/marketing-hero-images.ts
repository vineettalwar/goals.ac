...
const unsplash = (path: string, w = 1600) =>
  `https://images.unsplash.com/${path}?auto=format&fit=crop&w=${w}&q=65&fm=webp`;

const higgs = (url: string, w = 1200, q = 60) =>
  `https://images.higgs.ai/?default=1&output=webp&url=${encodeURIComponent(url)}&w=${w}&q=${q}&optimize=medium`;
