...
import { isStripeBillingActive, normalizePlanId, type PlanId } from "@workspace/billing";
import {
  organizationsTable,
  usersTable,
...
async function resolveBillingActor(userId: number): Promise<
  | {
      ok: true;
...
export async function handleBillingStatusGet(
  request: Request,
  userId: number,
  ...
  if (url.pathname.replace(/\/+$/, "") !== "/api/billing/status") return null;

  const actor = await resolveBillingActor(userId);
  if (!actor.ok) {
