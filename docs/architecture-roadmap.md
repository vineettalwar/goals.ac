...
| 4 | **Fragmented connector model** | WordPress creds per-publish (XML-RPC — deprecated and disabled on most managed hosts), Notion/Webflow in a JSONB column on `website_projects`, Ghost/Webhook in a separate `integration_connections` table. Three storage patterns, no common interface | 🟠 Blocks Phase 2 |
...
| 6 | **BYOK key handling is v1** | Single static secret (SHA-256 of one env var) encrypts *everything*: user AI keys, Notion tokens, Webflow tokens, Ghost keys. No key versioning, no rotation path, no audit log, keys decrypted in the web process. JWTs in localStorage, 30-day, non-revocable | 🟠 Trust/compliance risk |
...
- **Replace WordPress XML-RPC with the REST API + Application Passwords** immediately (XML-RPC is disabled by default on much of managed WP hosting). The Phase 2 plugin then *extends* the REST adapter rather than replacing it.
...
**Definition of done for Phase 1:** a new user can go from signup → goal → approved 30-day brief plan → first humanized article published to one connected CMS, with every AI call metered and billed against subscription or BYOK.
...
POST /goals-ac/v1/content         → create/update from PlatformPayload
POST /goals-ac/v1/schema          → inject JSON-LD / llms.txt
Auth: per-site key issued at pairing, HMAC-signed requests (webhook signer exists)
```
...
6. **Budgets**: every agent run draws from the credit ledger; orchestrator halts (and notifies) at budget exhaustion. BYOK users set monthly spend caps instead.
...
- **BYOK bypasses platform-model credits** (already implemented for quota) but still consumes *orchestration* credits at a reduced rate — the pipeline, storage, publishing, and agents-as-a-service are the product even when the tokens are the customer's. This keeps BYOK from cannibalizing revenue while preserving its cost-control pitch.
